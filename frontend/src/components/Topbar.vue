<script setup lang="ts">
import type { StatusPill } from '../app/state'
import type { State } from '../wailsbridge'
import { Button } from '@/components/ui/button'
import { Badge } from '@/components/ui/badge'

defineProps<{
  page: string
  state: State | null
  statusPill: StatusPill
}>()

defineEmits<{
  (e: 'refresh'): void
}>()
</script>

<template>
  <header class="flex flex-wrap items-start justify-between gap-4 rounded-2xl border border-border/60 bg-card/70 px-5 py-4 shadow-sm backdrop-blur">
    <div class="min-w-0 space-y-3">
      <div class="flex items-center gap-3">
        <Badge variant="outline" class="rounded-md text-[10px] uppercase tracking-[0.16em]">Console</Badge>
        <h2 class="truncate text-lg font-semibold tracking-tight text-foreground">{{ page }}</h2>
      </div>

      <div v-if="state" class="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
        <Badge variant="secondary" class="rounded-md font-mono">PID {{ state.pid || '-' }}</Badge>
        <Badge variant="secondary" class="rounded-md font-mono">{{ state.fullVersion || '-' }}</Badge>
        <Badge variant="secondary" class="max-w-full rounded-md">
          <span class="truncate">{{ state.lastSession || 'No Session' }}</span>
        </Badge>
        <Badge :variant="state.httpEnabled ? 'default' : 'outline'" class="rounded-md">HTTP</Badge>
        <Badge :variant="state.autoDecrypt ? 'default' : 'outline'" class="rounded-md">Auto Decrypt</Badge>
      </div>
    </div>

    <Button type="button" class="shrink-0" @click="$emit('refresh')">
      Refresh
    </Button>
  </header>
</template>
